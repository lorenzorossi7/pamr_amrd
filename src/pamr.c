...
//
// cp_file is a checkpoint file (on node 'cp_rank') to load a context from disk.
//=============================================================================
int PAMR_init_context(real *q, int q_size, int dim, int *shape, real *bbox, 
                      char *cp_file, int cp_rank)
{
   static int first=1;
   int i,cnt,ret=-10;
   int my_rank,mpi_size;
   int *levs,k,cnode,cnode_max,cp_err,g_cp_err,ltrace=1;
   real *p;
...
int PAMR_compose_hierarchy(int min_lev, int max_lev, int num, int *lev, real *bbox, real t)
{
   ...
   //--------------------------------------------------------------------------
   if (c->excision_on) init_ex_mask(min_lev,max_lev,new_cgh,c->amr_mask_gfn);
...
int PAMR_interp(int lc, int tl, int hierarchy)
{
   ...
   ret=communicate(lc,lc+1,hierarchy,0,1,0,"interp");
...
int PAMR_AMR_bdy_interp(int lc, int tl, int AMR_bdy_width)
{
   ...
   ret=communicate(lc,lc+1,PAMR_AMRH,0,1,AMR_bdy_width,"interp");
...
int PAMR_swap_tl(int lev, int tl1, int tl2)
{
   ...
   if (c->MG_cgh) { printf("PAMR_swap_tl: error ... cannot swap pointers when a MG hierarchy exists\n"); return 0; }
...
//                 PAMR_TF_INJECT_TO_MG_LEV(PAMR_MGH ONLY) or PAMR_TF_INTERPOLATE
//
...
void PAMR_set_tf_bits(int tl, int hierarchy, int stage)
{
   ...
   while(f)
   {
      ...
      for (i=gfs; i<gfe; i++) c->tf_bits[i-1]=op;
      f=f->next;
   }
...
//=============================================================================
// internal routine called by both PAMR_save_gfn() and PAMR_save_gfn_close()
//=============================================================================
int PAMR_do_save_gfn(char *name, int hier, int tl, int L, real t, char *pre_tag, char *post_tag, int close)
{
   ...
   IFG(1) printf(">> PAMR_save_gfn(%s,...,close=%i)\n",name,close);
...
//=============================================================================
// saves specified grid function to (calls above function
//
                     ...
                     real excised, int initialize_now)
{
   ...
   IFG(1) printf(">> PAMR_excision_on(%s,...)\n",ex_mask_var);
...
// check-point routine
//=================================================================================
int PAMR_cp(char *cp_file, int cp_rank)
{
   context *c=curr_context;
