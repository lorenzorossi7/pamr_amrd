	       ...
	       FTYPE gdet, FTYPE prim[NPR], FTYPE GAMMA)
{
  ...
  static int Utoprim_new_body(FTYPE U[], FTYPE gcov[NDIM][NDIM], 
			      FTYPE gcon[NDIM][NDIM], FTYPE gdet,  FTYPE prim[], FTYPE GAMMA);

  FTYPE U_tmp[NPR], U_tmp2[NPR], prim_tmp[NPR];
  int i, j, ret; 
  FTYPE alpha;

/*   printf("inside Utoprim_2d\n"); */
/*   for (i=0; i<NPR; i++) { */
/*     printf("i = %d, prim[i] = %f\n",i,prim[i]); */
/*   } */
...
/*   } */
  ...
  ret = Utoprim_new_body(U_tmp, gcov, gcon, gdet, prim_tmp, GAMMA);
...
static int Utoprim_new_body(FTYPE U[NPR], FTYPE gcov[NDIM][NDIM], 
			    FTYPE gcon[NDIM][NDIM], FTYPE gdet,  FTYPE prim[NPR], FTYPE GAMMA)
{
  ...
  static int general_newton_raphson( FTYPE x[], int n, FTYPE GAMMA,
				     void (*funcd) (FTYPE [], FTYPE [], 
						    FTYPE [], FTYPE [][NEWT_DIM], 
						    FTYPE *, FTYPE *, int, FTYPE) );

  static  void func_vsq( FTYPE [], FTYPE [], FTYPE [], FTYPE [][NEWT_DIM], 
			 FTYPE *f, FTYPE *df, int n, FTYPE GAMMA);
  static FTYPE x1_of_x0(FTYPE x0 ) ;


  //printf("inside new body, prim[rho] = %f\n",prim[RHO]);
  ...
  rho0 = D / gamma ;
  u = prim[UU] ;
  p = pressure_rho0_u(rho0,u,GAMMA) ;
  w = rho0 + u + p ;
  ...
  retval = general_newton_raphson( x_2d, n, GAMMA, func_vsq ) ;  
					   ...
					   FTYPE *, int, FTYPE) )
{
  ...
  while( keep_iterating ) { 
    ...
    (*funcd) (x, dx, resid, jac, &f, &df, n, GAMMA);  /* returns with new dx, f, df */
...
static void func_vsq(FTYPE x[], FTYPE dx[], FTYPE resid[], 
		      FTYPE jac[][NEWT_DIM], FTYPE *f, FTYPE *df, int n, FTYPE GAMMA)
{
  ...
  FTYPE t40;
  FTYPE t9;
  static FTYPE pressure_W_vsq(FTYPE W, FTYPE vsq, FTYPE GAMMA) ;
  static FTYPE dpdW_calc_vsq(FTYPE W, FTYPE vsq, FTYPE GAMMA);
  static FTYPE dpdvsq_calc(FTYPE W, FTYPE vsq, FTYPE GAMMA);
  ...
  p_tmp  = pressure_W_vsq( W, vsq, GAMMA);
  dPdW   = dpdW_calc_vsq( W, vsq, GAMMA );
  dPdvsq = dpdvsq_calc( W, vsq, GAMMA );
