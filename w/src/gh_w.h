...
/* the following global defines which gfn's are cell-centered and which vertex
 * centered ... calculated after PAMR_compose_hierarhcy */
...
typedef struct grid
{
   ...
   struct grid *prev; 
   int dim;
   int shape[PAMR_MAX_DIM];       /* the shape of vertex centered grids */
   int shape_c[PAMR_MAX_DIM];     /* the shape of cell centered centered grids - V2 */
   ...
   real t;
   real *x[PAMR_MAX_DIM];         /* pointers to perimeter coordinate arrays for vertex centered grids */
   real *x_c[PAMR_MAX_DIM];       /* pointers to perimeter coordinate arrays for cell centered grids - V2 */
...
typedef struct context
{
   ...
   int amr_mask_gfn;             
   int mg_mask_c_gfn;               /* mask variable grid function numbers for cell-centered vars - V2*/
   int amr_mask_c_gfn;              /* V2 */
   int *gfn_var_type;               /* V2 --- array of integers telling the type of variables */
} context;

extern context *curr_context;
extern context *contexts[PAMR_MAX_CONTEXTS];
extern char *PAMR_c_tag,*PAMR_v_tag;
