...
char *PAMR_c_tag=0,*PAMR_v_tag=0;
...
var *new_var(char *name)
{
   ...
   if (!f || !new_name)
   { 
      printf("new_var: error ... out of memory f=%p, new_name=%p\n",f,new_name); 
      if (f) free(f);
...
//=============================================================================
// finds the name of the variable corresponding to a given gfn
//=============================================================================
char *find_name(int gfn)
{
   var *f=curr_context->vars;
   int num;
   ...
   while(f)
   {
      num=f->num_tl; if (f->in_mgh) num++;
      if (gfn >= f->sgfn && gfn < f->sgfn+num) return f->name;
      f=f->next;
   }

   printf("find_name: error ... invalid gfn=%i\n",gfn);
   return 0;
}
...
void insert_grid(level *lev, grid *g)
{
   ...
   while(cg && gridcmp(g,cg)>0) {pg=cg; cg=cg->next;}
...
cgh *build_sgh(int min_lev, int max_lev, int num, int *lev, real *bbox, real t)
{
   ...
   for (i=0; i<num; i++)
   {
	 ...
	 if (g->shape[j] > 1) {
	   g->shape_c[j]=g->shape[j]-1;
	 } else {
	   g->shape_c[j]=g->shape[j];
	 }
         g->x[j]=0;
         g->x_c[j]=0;
         g->ghost_width[2*j]=0;
      ...
      if (lev=c->levels[l])
      {
         ...
         while(g)
         {
            ...
            if (!q->virtual_g)
            {
               ...
               for (i=0; i<curr_context->dim; i++) if (q->x_c[i]) pfree(q->x_c[i]);
            }
            free(q);
         }
         free(lev);
...
int block_size_c(grid *g)
{
   int size=0,i;

   size=g->shape_c[0];
   for (i=1; i<g->dim; i++)  size*=g->shape_c[i];
   return size;
}
   ...
   for (l=sgh->min_lev-1; l<=(sgh->max_lev-1); l++)
   {
	...
	IFG0(4) printf("level %i:\n\n",l+1);
         ...
         while(g)
         {
            ...
            //
            // Note: for split sized we don't consider differences between
            // cell centered/vertex vertices
            //--------------------------------------------------------------------
            ...
            for (i=0; i<nx; i++)
            {
               for (j=0; j<ny; j++)
               {
                  for (k=0; k<nz; k++)
                  {
                     ...
                     // much as possible):
		     // Branson:  Commented out this statement in order to simplify
		     // the interpretation of ghost width for grids with data at
		     // vertices and cell centers.
			...
			// Branson:  Commented out this statement.
		     ...
		     IFG0(4) switch(dim)
		       {
                        ...
                        case 3: printf("segment %i bbox=[%16.10lf,%16.10lf][%16.10lf,%16.10lf][%16.10lf,%16.10lf], rank=%i\n",i+j*nx+k*nx*ny,
                                ...
                                printf("\t ghostwidth=[%i,%i][%i,%i][%i,%i]\n",ng->ghost_width[0],ng->ghost_width[1],
                                                                               ng->ghost_width[2],ng->ghost_width[3],
                                                                               ng->ghost_width[4],ng->ghost_width[5]);
                                break;
		       }

                     for (h=0;h<c->dim; h++) ng->x[h]=ng->x_c[h]=0;
                     ng->gfs=0;
                     //-----------------------------------------------------------
                     ...
                     if (ng->rank==c->rank)
                     {
                        ...
                        for (h=0;h<ng->ngfs;h++)
                        {
                               ...
                               ((!alloc_mg && is_in_amrh(h+1)) || (alloc_mg && !(is_in_amrh(h+1)))) )
                           {
                              ...
                              if (c->gfn_var_type[h]==PAMR_VERTEX_CENTERED)
                              {
                              ...
                              {
                                 if (!(ng->gfs[h]=(real *)pmalloc(sizeof(real)*block_size_c(ng))))
                                    {printf("compose_cgh: error ... out of mem\n"); goto error;}
                                 for (m=0; m<block_size_c(ng); m++) (ng->gfs[h])[m]=0;
                              }
                           }
                        }
                        for (h=0;h<c->dim; h++)
                        {
                           if (!(ng->x[h]=(real *)pmalloc(sizeof(real)*ng->shape[h])))
                              {printf("compose_cgh: error ... out of mem\n"); goto error;}
                           if (!(ng->x_c[h]=(real *)pmalloc(sizeof(real)*ng->shape_c[h])))
                              {printf("compose_cgh: error ... out of mem\n"); goto error;}
                           dx=(ng->bbox[2*h+1]-ng->bbox[2*h])/(ng->shape[h]-1);
                           for (m=0,x=ng->bbox[2*h]; m<ng->shape[h]; m++,x+=dx) (ng->x[h])[m]=x;
                           for (m=0,x=ng->bbox[2*h]+dx/2; m<ng->shape_c[h]; m++,x+=dx) (ng->x_c[h])[m]=x;
                        }
                     }
                     insert_grid(lev,ng);
                  }
...
void init_new_cgh(cgh *new_cgh,cgh *old_cgh)
{
   ...
   for (l=new_cgh->min_lev-1; l<=(new_cgh->max_lev-1); l++)
   {
      if (new_cgh->levels[l])
      {
         ...
         for (i=0; i<c->size; i++)
         {
            if (l>=1 && old_cgh->levels[l-1]) src_int[i]=build_owned_gsl(i,old_cgh,l,1,1);
            if (old_cgh->levels[l]) src_trans[i]=build_owned_gsl(i,old_cgh,l+1,1,0);  
         }
         IFG(3) 
         {
            if (src_int[c->rank]) 
...
int incorporate_new_cgh(cgh *new_cgh, cgh *curr_cgh)
{
   ...
   IFG(2)
   {
      ...
      for (l=0; l<PAMR_MAX_LEVS; l++) printf("l=%i. new[l]=%p, old[l]=%p\n",l,new_cgh->levels[l],curr_cgh->levels[l]);
   }
   ...
   for (l=max_lev-1; l>=(min_lev-1); l--)
   {
      ...
      while(n)
      {
         ...
         if (num)
         {
            ...
            if (c->excision_on) init_ex_mask(sgh->min_lev,sgh->max_lev,new_cgh,c->mg_mask_gfn,c->mg_mask_c_gfn);
   ...
   for (l=mgh->min_lev-1; l<=(mgh->max_lev-1); l++)
   {
      if (lev=mgh->levels[l])
      {
         ...
         while(g)
         {
            ...
            for (i=0; i<curr_context->dim; i++) if (q->x_c[i] && !mgh->in_amrh[l]) pfree(q->x_c[i]);
            free(q);
         }
         free(lev);
      }
...
//=============================================================================
void init_ex_mask(int Lmin, int Lmax, cgh *gh, int gfn, int gfn_c)
{
   context *c=curr_context;
   int L;
   grid *g;
   ...
   for (L=Lmin; L<=Lmax; L++)
   {
      ...
      while(g)
      {
         if (g->rank==c->rank)
         {
            if (!(g->gfs[gfn-1]) && !(g->gfs[gfn_c-1])) 
            {
               printf("init_ex_mask: error ... mask grid not allocated\n"); exit(1);
            }
            c->app_fill_ex_mask(g->gfs[gfn-1],g->gfs[gfn_c-1],g->dim,g->shape,g->shape_c,g->bbox,c->excised);
         }
         g=g->next;
      }
   }
...
//=============================================================================
// given a variable name, the following returns a pointer to the var 
// structure of it's "conjugate" (cell <->  vertex), if it exists
//=============================================================================
var *find_conj_var(char *name)
{
   context *c=curr_context;
   ...
   {
      printf("find_conj_var: error ... neither PAMR_c_tag nor PAMR_v_tag defined\n");
      return 0;
   }
   ...
   if ((1+slen+min(clen,vlen))>512)
   {
      printf("find_conj_var: buf to small\n");
      return 0;
   }

   strcpy(buf,name);

   if (PAMR_var_type(name)==PAMR_VERTEX_CENTERED)
   {
      if (PAMR_v_tag && slen>vlen && !(strcmp(PAMR_v_tag,&name[slen-vlen]))) 
      {
         buf[slen-vlen]=0; 
         if (PAMR_c_tag!=0 && (ret=find_var(buf))) return ret; 
      }
      if (PAMR_c_tag) strcat(buf,PAMR_c_tag);
      if (ret=find_var(buf)) return ret; 
   }
   else
   {
      if (PAMR_c_tag && slen>clen && !(strcmp(PAMR_c_tag,&name[slen-clen]))) 
      {
         buf[slen-clen]=0; 
         if (ret=find_var(buf)) return ret; 
      }
      if (PAMR_v_tag) strcat(buf,PAMR_v_tag);
      if (ret=find_var(buf)) return ret; 
   }
...
//=============================================================================
// for i=0..gfn-1, sets conj_gfns[i]=the conjugate grid function number to gfn=i
//=============================================================================
void fill_conjugates(int *conj_gfns)
{
   int i,j, min_num_tl;
   var *v,*cv;

   for (i=0; i<curr_context->gfns; i++) conj_gfns[i]=0;

   v=curr_context->vars;
   while(v)
   {
      cv=find_conj_var(v->name);
      if (cv)
	{
	  min_num_tl = min(v->num_tl,cv->num_tl); 
	  for(i=0; i<min_num_tl; i++) conj_gfns[v->sgfn+i-1]=cv->sgfn+i;

	  if (v->num_tl > min_num_tl) {
	    for(i=min_num_tl; i<v->num_tl; i++) conj_gfns[v->sgfn+i-1]=cv->sgfn+min_num_tl-1;
	  }

          if (v->in_mgh && cv->in_mgh) conj_gfns[v->sgfn+v->num_tl-1]=cv->sgfn+cv->num_tl;
	}
      v = v->next;
   }
}
...
{
   real dx[PAMR_MAX_DIM],width[PAMR_MAX_DIM];
   real g1_bbox[2*PAMR_MAX_DIM],g2_bbox[2*PAMR_MAX_DIM]; 
   context *c=curr_context;
   ...
   for(i=0; i<g1->dim; i++)
   {
      dx[i]=(g1->bbox[2*i+1]-g1->bbox[2*i])/g1->shape[i];

      if (fuzz_lt(g1->bbox[2*i],g2->bbox[2*i],dx[i]/2)) return -1;
      if (fuzz_gt(g1->bbox[2*i],g2->bbox[2*i],dx[i]/2)) return 1;
   }
