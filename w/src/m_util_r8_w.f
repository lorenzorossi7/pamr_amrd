...
c
c ord is PAMR type (see ops.inc)
c
...
c---------------------------------------------------------------------------
     ...
     &                       chr1,ex,do_ex)
        ...
        if (.not.(ord.eq.PAMR_SECOND_ORDER.or.
     &            ord.eq.PAMR_FOURTH_ORDER)) then
           write (*,*) 'dminterp3d: only 2nd,4th order supported'
           stop
        end if 
     ...
     &                si1,sj1,sk1,irho,jrho,krho)
        ...
        do j=j2,j2+(nj-1)*jrho,jrho
           do k=k2,k2+(nk-1)*krho,krho
              if (ord.eq.PAMR_SECOND_ORDER) then
                 if (do_ex.eq.1) then
...
c---------------------------------------------------------------------------
c half-weight restriction with direct injection on boundary (if s1=[2,2,2]): 
c dmhwrbdy3d copies a segment of grid u1, with half-weight averaging in the 
c interior and direct injection on the boundary, into grid u2.
c 
c offset in u1 = [i1,j1,k1] with stride [si1,sj1,sk1],
c offset in u2 = [i2,j2,k2] with stride [si2,sj2,sk2]
c size of segment=ni x nj x nk in u2
...
c if (do_ex==1), then excise
c---------------------------------------------------------------------------
        subroutine dmhwrbdy3d(u1,u2,nx1,ny1,nz1,nx2,ny2,nz2,
     &                     i1,j1,k1,i2,j2,k2,ni,nj,nk,
     &                     si1,sj1,sk1,si2,sj2,sk2,
     &                     chr1,ex,do_ex)
        implicit none
     ...
     &                    u1(i0,j0,k0-1)+2*u1(i0,j0,k0)+u1(i0,j0,k0+1)
                 ...
                 if (do_ex.eq.1.and.chr1(i0,j0,k0).eq.ex) then
                    u2(i2+(i-1)*si2,j2+(j-1)*sj2,k2+(k-1)*sk2)=0
                 else if ( (w.eq.12).or.(nz1.eq.1.and.w.eq.8)
     &                    .or.(nz1.eq.1.and.ny1.eq.1.and.w.eq.4) ) then !- whether or not on boundary
                    u2(i2+(i-1)*si2,j2+(j-1)*sj2,k2+(k-1)*sk2)=sum/w
                 else
                    u2(i2+(i-1)*si2,j2+(j-1)*sj2,k2+(k-1)*sk2)=
     &              u1(i0,j0,k0)
...
c        parameter (w4=1.0d0/16.0d0,w3=1.0d0/16.0d0,w2=1.0d0/16.0d0)
     ...
     &               (chr(i,j,k).ne.ex)) then
                   ...
                   if (do_ex.lt.0) eps_eff=abs(eps(i,j,k))
...
c
c in use_6th *OR* stride>1 then calls dmdiss3d_64th
c NOTE: dmdiss3d_64th uses different boundary stencils
c
c NOTE: update DV version too 
c
c stride is step size (nomally 1), i.e. Up(f) = f(i+stride)-f(i), etc. 
c---------------------------------------------------------------------------
        subroutine dmdiss3d(f,work,eps,do_bdy,phys_bdy_type,even,odd,
     &                      mask,mask_off,nx,ny,nz,chr,ex,do_ex,use_6th,
     &                      stride)
        implicit none
        integer nx,ny,nz,do_ex,do_bdy,phys_bdy_type(6),even,odd,use_6th
        integer stride
        real*8 f(nx,ny,nz),work(nx,ny,nz),mask(nx,ny,nz),eps(nx,ny,nz)
        real*8 mask_off
        real*8 chr(nx,ny,nz),ex
...
c        parameter (w4=1.0d0/16.0d0,w3=1.0d0/16.0d0,w2=1.0d0/16.0d0)
        ...
        if (use_6th.ne.0) then
           call dmdiss3d_64th(f,work,eps,do_bdy,phys_bdy_type,even,odd,
     &          mask,mask_off,nx,ny,nz,chr,ex,do_ex,6,stride)
           return
        else if (stride.gt.1) then
           call dmdiss3d_64th(f,work,eps,do_bdy,phys_bdy_type,even,odd,
     &          mask,mask_off,nx,ny,nz,chr,ex,do_ex,4,stride)
           return
        end if
...
c---------------------------------------------------------------------------
c 4th/6th order dissipation (as specified by ord = 4 or 6)
c
c *NOTE*: different from dmdiss3d_ex in that only even derivative powers
c are used. Minus signs come in because Fourier transform
c of d^n f/dx^n = (i*w)^n F(w), plus (i suspect) phase shifts for
c offset operators
c
c *TEST* : if internal flag use_12_21 then uses third derivate operator
c          near excision boundaries as well
c
c applies a simple KO filter to f, where mask != mask_off, and modified
c near boundaries if do_bdy=1 as follows
c
c with undivided operators Up(f) = f(i+1)-f(i)
c                          Um(f) = f(i)-f(i-1)
c
c (L,R) : (# of usable points to left, # of usable points to right):
c
c if 6th:
c
c (3,3) : -eps*w6*(Up Um)^3 : -(f(i+3) -6f(i+2) +15f(i+1) -20f(i) +15f(i-1) -6f(i-2) +f(i-3))
c
c if 4th or (do_bdy & 6th):
c
c (2,2) :  eps*w4*(Up Um)^2 :            f(i+2) - 4f(i+1) + 6f(i) - 4f(i-1) + f(i-2)
c
c if (do_bdy):
c
c IF (use_12_21): [will generically supercede (1,1) ]
c   (1,2) : -eps*w3*(Up Um Um)   :               (-f(i+1) + 3f(i) - 3f(i-1) +  f(i-2))
c   (2,1) : -eps*w3*(Up Up Um)   :      (f(i+2)  -3f(i+1) + 3f(i) -  f(i-1))
c (1,1) : -eps*w2*(Up Um)   :                    -(f(i+1) - 2f(i) +  f(i-1))
c (0,2) :  eps*w2*(Up Up)   :           (f(i+2) - 2f(i+1) +  f(i))
c (2,0) :  eps*w2*(Um Um)   :                               (f(i) - 2f(i-1) +  f(i-2))
c
c NOTE: SPECIAL FLAG ... if do_ex>0, eps is treated as a scalar, 
c       else eps is treated as an array.
c
c independent sweeps in x/y/z done here
c
c stride is step size (nomally 1), i.e. Up(f) = f(i+stride)-f(i), etc. 
c
c SPECIAL FLAG: of do_ex<0, and eps<0, only use 4th order at that point
c------------------------------------------------------------------------
        subroutine dmdiss3d_64th(f,work,eps,do_bdy,phys_bdy_type,even,
     &                          odd,mask,mask_off,nx,ny,
     &                          nz,chr,ex,do_ex,ord,stride)
        implicit none
        integer nx,ny,nz,do_bdy,phys_bdy_type(6),even,odd,do_ex,stride
        integer ord
        real*8 f(nx,ny,nz),work(nx,ny,nz),mask(nx,ny,nz),eps(nx,ny,nz)
        real*8 mask_off
        real*8 chr(nx,ny,nz),ex

        integer i,j,k,bo1,bo2,bo3,h,i0,stn,j0,k0,pass,npass,s,i_cn,h_mx
        real*8 eps_eff,f_hf,norm_f,pm,chr0
        real*8 w4,w3,w2,w5,w6,wf(7),cf(7),maskf(7)
        logical use_12_21
        parameter (w6=1.0d0/64.0d0,w5=1.0d0/32.0d0,use_12_21=.true.)
        parameter (w4=1.0d0/16.0d0,w3=1.0d0/8.0d0,w2=1.0d0/4.0d0)
        ...
        if (s.lt.1) then
           write(*,*) 'dmdiss3d_64th: error ... stride < 1'
           stop
        end if
        ...
        if (ord.eq.4) then
           i_cn=3
           h_mx=5
        else if (ord.eq.6) then
           i_cn=4
           h_mx=7
        else
           write(*,*) 'dmdiss3d_64th: error ... ord must be 4 or 6'
           stop
        end if
         ...
         do i=1,nx
           do j=1,ny
              do k=1,nz
                ...
                if (mask(i,j,k).ne.mask_off.and.chr0.ne.ex) then
                  ...
                  if (pass.eq.1) then
                    do h=1,h_mx
                     pm=1
                     i0=i-s*(i_cn-h)
                     if (i0.lt.1.and.phys_bdy_type(1).eq.odd) then
                        pm=-1
                        i0=-i0+2
                     else if (i0.lt.1.and.phys_bdy_type(1).eq.even) then
                        i0=-i0+2
                     else if (i0.gt.Nx.and.phys_bdy_type(2).eq.odd) then
                        pm=-1
                        i0=Nx-(i0-Nx)
                     else if (i0.gt.Nx.and.phys_bdy_type(2).eq.even)then
                        i0=Nx-(i0-Nx)
                     end if
                     if (i0.lt.1.or.i0.gt.Nx) then
                        wf(h)=0
                        maskf(h)=mask_off
                        cf(h)=ex
                     else
                     ...
                     if (h.eq.i_cn.and.maskf(h).eq.mask_off) cf(h)=ex
                    end do
                   ! y 
                    ...
                    do h=1,h_mx
                     pm=1
                     j0=j-s*(i_cn-h)
                     if (j0.lt.1.and.phys_bdy_type(3).eq.odd) then
                        pm=-1
                        j0=-j0+2
                     else if (j0.lt.1.and.phys_bdy_type(3).eq.even) then
                        j0=-j0+2
                     else if (j0.gt.Ny.and.phys_bdy_type(4).eq.odd) then
                        pm=-1
                        j0=Ny-(j0-Ny)
                     else if (j0.gt.Ny.and.phys_bdy_type(4).eq.even)then
                        j0=Ny-(j0-Ny)
                     end if
                     if (j0.lt.1.or.j0.gt.Ny) then
                        wf(h)=0
                        maskf(h)=mask_off
                        cf(h)=ex
                     else
                   ...
                   ! z 
                    ...
                    do h=1,h_mx
                     pm=1
                     k0=k-s*(i_cn-h)
                     if (k0.lt.1.and.phys_bdy_type(5).eq.odd) then
                        pm=-1
                        k0=-k0+2
                     else if (k0.lt.1.and.phys_bdy_type(5).eq.even) then
                        k0=-k0+2
                     else if (k0.gt.Nz.and.phys_bdy_type(6).eq.odd) then
                        pm=-1
                        k0=Nz-(k0-Nz)
                     else if (k0.gt.Nz.and.phys_bdy_type(6).eq.even)then
                        k0=Nz-(k0-Nz)
                     end if
                     if (k0.lt.1.or.k0.gt.Nz) then
                        wf(h)=0
                        maskf(h)=mask_off
                        cf(h)=ex
                     else
                   ...
                   ! using i0 for all, to allow common dissipation code

                   i0=i_cn
                   ! (3,3) : -w6*(f(i+3) -6f(i+2) +15f(i+1) -20f(i) +15f(i-1) -6f(i-2) +f(i-3))
                   if (ord.eq.6..and.eps_eff.gt.0.and.
     &                 (cf(i0-3).ne.ex.and.cf(i0-2).ne.ex.and.
     &                  cf(i0-1).ne.ex.and.cf(i0).ne.ex.and.
     &                  cf(i0+1).ne.ex.and.cf(i0+2).ne.ex.and.
     &                  cf(i0+3).ne.ex)) then
                      f_hf=-w6*(wf(i0-3)+wf(i0+3)-6*(wf(i0-2)+wf(i0+2))
     &                           +15*(wf(i0-1)+wf(i0+1))-20*wf(i0))
                   ! (2,2) : w4*(f(i+2) - 4f(i+1) + 6f(i) - 4f(i-1) + f(i-2))
                   else if ((ord.eq.4.or.do_bdy.ne.0.or.eps_eff.lt.0)
     &                 .and.
     &                 cf(i0-2).ne.ex.and.cf(i0-1).ne.ex.and.
     &                 cf(i0).ne.ex.and.cf(i0+1).ne.ex.and.
     &                 cf(i0+2).ne.ex) then
                      f_hf=w4*(wf(i0+2)-4*wf(i0+1)+6*wf(i0)
     &                                 -4*wf(i0-1)+wf(i0-2))
                   !   (1,2) : w3*(-f(i+1) + 3f(i) - 3f(i-1) +  f(i-2))
                   else if (do_bdy.ne.0.and.use_12_21.and.
     &                 cf(i0-2).ne.ex.and.cf(i0-1).ne.ex.and.
     &                 cf(i0).ne.ex.and.cf(i0+1).ne.ex) then
                      f_hf=w3*(-wf(i0+1)+3*wf(i0)-3*wf(i0-1)+wf(i0-2))
                   !   (2,1) : w3*(f(i+2)  -3f(i+1) + 3f(i) -  f(i-1))
                   else if (do_bdy.ne.0.and.use_12_21.and.
     &                 cf(i0-1).ne.ex.and.cf(i0).ne.ex.and.
     &                 cf(i0+1).ne.ex.and.cf(i0+2).ne.ex) then
                      f_hf=w3*(-wf(i0-1)+3*wf(i0)-3*wf(i0+1)+wf(i0+2))
                   ! (1,1) : -w2*(f(i+1) - 2f(i) +  f(i-1))
                   else if (do_bdy.ne.0.and.
     &                 cf(i0-1).ne.ex.and.cf(i0).ne.ex.and.
     &                 cf(i0+1).ne.ex) then
                      f_hf=-w2*(wf(i0+1)-2*wf(i0)+ wf(i0-1))
                   ! (2,0) : w2*(f(i-2) - 2f(i-1) +  f(i))
                   else if (do_bdy.ne.0.and.
     &                 cf(i0-2).ne.ex.and.cf(i0).ne.ex.and.
     &                 cf(i0-1).ne.ex) then
                      f_hf=w2*(wf(i0-2)-2*wf(i0-1)+ wf(i0))
                   ! (0,2) : w2*(f(i) - 2f(i+1) +  f(i+2))
                   else if (do_bdy.ne.0.and.
     &                 cf(i0+2).ne.ex.and.cf(i0).ne.ex.and.
     &                 cf(i0+1).ne.ex) then
                      f_hf=w2*(wf(i0+2)-2*wf(i0+1)+ wf(i0))
                   ...
                   f(i,j,k)=f(i,j,k)-abs(eps_eff)*f_hf
                end if ! mask off
              end do ! k
            end do ! j
          end do ! i
