...
//        : TRE_ibc_buffer ... interior-boundary-clear buffer, i.e. the region 
//          adjacent to interior boundaries (computed over the union of grids
//          on a level) overwhich to zero the TRE. May not work properly
//          if TRE_ibc_buffer > minimum size of a grid. 
...
void calc_tre(int L1, int L2)
{
   ...
   for (L=L1; L<=L2; L++)
   {
      IFL for (i=0; i<AMRD_num_f_tre_vars; i++) PAMR_save_gfn(AMRD_f_tre_vars[i],PAMR_AMRH,1,L,lcount,"debug_","_bare");

      valid=PAMR_init_s_iter(L,PAMR_AMRH,0);
      while(valid)
      {
         ev_ldptr();
         for (i=0; i<AMRD_g_size; i++) AMRD_tre[i]=0;
         valid=PAMR_next_g();
      }
      ...
      if (AMRD_TRE_ibc_buffer>0)
      {
         ...
         set_gfn_sync(AMRD_cmask_gfn);
         PAMR_sync(L,0,PAMR_AMRH,0);
         //--------------------------------------------------------------------
         // in case there are overlapping (sequential) grids
         //--------------------------------------------------------------------
         if (AMRD_TRE_ibc_buffer>0) extend_cmask_bdy(L,PAMR_AMRH,AMRD_TRE_ibc_buffer); 
      ...
      IFL for (i=0; i<AMRD_num_f_tre_vars; i++) PAMR_save_gfn(AMRD_f_tre_vars[i],PAMR_AMRH,1,L,lcount,"debug_","_after_ibc");
   }
   ...
   for (L=L2; L>=L1; L--)
   {
      ...
      if (L<L2) 
      {
         ...
         set_gfn_in(AMRD_tre_gfn,PAMR_STRAIGHT_INJECT);
      ...
      {
         ev_ldptr();
         if (exc_buf0) bleed_tre(exc_buf0,-1); 
         bleed_tre(l_TRE_buffer,1);
         if (!exc_buf0) for (i=0; i<AMRD_g_size; i++) if (AMRD_tre[i]>0.5) AMRD_tre[i]=1; else AMRD_tre[i]=0;
         valid=PAMR_next_g();
      }
      PAMR_sync(L,0,PAMR_AMRH,0);
      ...
      IFL PAMR_save_gfn("AMRD_tre",PAMR_AMRH,1,L,lcount,"debug_","_flagged");
   }
...
void simple_cls(int L1, int L2, real **gbbox, int **glev, int *gnum)
{
   ...
   *gnum=0;
   ...
   for (L=L1; L<=L2; L++)
   {
      ...
      {
         ev_ldptr();
         while(find_seed(&i0,&j0,&k0)) { if (add_cluster(i0,j0,k0,lc)) {lc+=2*AMRD_dim; lnum++;} }
         valid=PAMR_next_g();
      }

      IFL debug_save_bbox_list(lbbox,0,L,lnum,lcount,"simple_cls_bf_merge");
      ...
      IFL printf("rank=%i, lgnum after PAMR_merge_bboxes=%i\n",my_rank,lgnum);

      IFL debug_save_bbox_list(gc,0,L,lgnum,lcount,"simple_cls_af_PAMR_merge");
      ...
      clean_cls(gc,&lgnum,L);

      IFL printf("rank=%i, lgnum after merge_cls=%i\n",my_rank,lgnum);

      IFL debug_save_bbox_list(gc,0,L,lgnum,lcount,"simple_cls_af_merge");
...
//=============================================================================
// cleans up the cluster list ... currently that only means deleting grids
// that are too small
//=============================================================================
...
void merge_cls(real *bbox, int *num, int lev)
{
   ...
   IFL printf("\tmerge_cls: rank=%i, AMRD_cls_merge_dist=%i, num=%i, AMRD_dim=%i, dx=[%lf,%lf,%lf]\n",
...
void extend_ex_cls(real *bbox, int num, int lev, real *bh_bbox, int num_bhs)
{
   ...
   IFL printf("extend_ex_cls: num=%i, num_bhs=%i\n",num,num_bhs);
...
void adjust_cls(int L1, int L2, real **gbbox, int **glev, int *gnum)
{
   ...
   *gnum=num;
   ...
   IFL0 debug_save_bbox_list(*gbbox,*glev,0,*gnum,lcount,"adjust_cls_");
...
void zero_f_tre(int L)
{
   ...
   while(valid)
   {
      ev_ldptr();
      for (i=0; i<AMRD_g_size; i++) 
         for (j=0; j<AMRD_num_f_tre_vars; j++) (AMRD_f_tre[j])[i]=0;
      valid=PAMR_next_g();
   }
}
