...
real vcycle(int giter, int save_tre)
{
   ...
   for (L=Lf; L>0; L--)
   {
      if (L<Lf)
      {
         ...
         // for subsequent cgc calculations ... essential for MG convergence
         //--------------------------------------------------------------------
...
void mg_ldptr()
{
   ...
   if (AMRD_do_ex) AMRD_chr_mg=gfs[AMRD_chr_mg_gfn-1]; else AMRD_chr_mg=AMRD_cmask_mg;
}
...
void compute_cgc(int L)
{
   ...
   while(valid)
   {
      ...
      for (i=0; i<num; i++) 
      {
         for (j=0; j<AMRD_g_size; j++) (AMRD_MG_f[i])[j]+=(AMRD_MG_res[i])[j];
      }
      valid=PAMR_next_g();
   }
}
...
void set_cmask_bdy(int L,int hier)
{
   ...
   while(valid)
   {
      ...
      is=js=ks=je=ke=0;
      ie=AMRD_g_shape[0]-1;
      if ((AMRD_g_bbox[0]-AMRD_base_bbox[0])>AMRD_g_dx[0]/2 || AMRD_periodic[0]) is++;
      if ((AMRD_base_bbox[1]-AMRD_g_bbox[1])>AMRD_g_dx[0]/2 || AMRD_periodic[0]) ie--;
      if (AMRD_g_dim>1)
      {
         je=AMRD_g_shape[1]-1;
         if ((AMRD_g_bbox[2]-AMRD_base_bbox[2])>AMRD_g_dx[1]/2 || AMRD_periodic[1]) js++;
         if ((AMRD_base_bbox[3]-AMRD_g_bbox[3])>AMRD_g_dx[1]/2 || AMRD_periodic[1]) je--;
         if (AMRD_g_dim>2)
         {
            ke=AMRD_g_shape[2]-1;
            if ((AMRD_g_bbox[4]-AMRD_base_bbox[4])>AMRD_g_dx[2]/2 || AMRD_periodic[2]) ks++;
            if ((AMRD_base_bbox[5]-AMRD_g_bbox[5])>AMRD_g_dx[2]/2 || AMRD_periodic[2]) ke--;
         }
      }
      if (is>=0 && js>=0 && ks>=0 && is<=ie && js<=je && ks<=ke &&
          ie<AMRD_g_shape[0] && je<AMRD_g_shape[1] && ke<AMRD_g_shape[2])
...
void inject_cnst_data(int Lf)
{
   ...
   for (i=0; i<AMRD_num_MG_cnst_data_vars; i++) PAMR_set_tf_bit(AMRD_MG_cnst_data_gfn[i],PAMR_STRAIGHT_INJECT);
...
void interp_cnst_data(int Lc)
{
   ...
   for (i=0; i<AMRD_num_MG_cnst_data_vars; i++) PAMR_set_tf_bit(AMRD_MG_cnst_data_gfn[i],PAMR_FOURTH_ORDER);
...
void mg_dump(int L, char *tag, int giter, int which)
{
   ...
   if (which<2)
   {
      for (i=0; i<num; i++) 
      {
         if (i<AMRD_num_elliptic_vars)
         {
            strcpy(buf,AMRD_elliptic_vars[i]); strcat(buf,"_lop"); PAMR_save_gfn(buf,PAMR_MGH,0,L,t,tag,"");
            strcpy(buf,AMRD_elliptic_vars[i]); strcat(buf,"_rhs"); PAMR_save_gfn(buf,PAMR_MGH,0,L,t,tag,"");
            strcpy(buf,AMRD_elliptic_vars[i]); strcat(buf,"_res"); PAMR_save_gfn(buf,PAMR_MGH,0,L,t,tag,"");
         }
         else
         {
            strcpy(buf,AMRD_elliptic_vars_t0[i-AMRD_num_elliptic_vars]); strcat(buf,"_lop"); PAMR_save_gfn(buf,PAMR_MGH,0,L,t,tag,"");
            strcpy(buf,AMRD_elliptic_vars_t0[i-AMRD_num_elliptic_vars]); strcat(buf,"_rhs"); PAMR_save_gfn(buf,PAMR_MGH,0,L,t,tag,"");
            strcpy(buf,AMRD_elliptic_vars_t0[i-AMRD_num_elliptic_vars]); strcat(buf,"_res"); PAMR_save_gfn(buf,PAMR_MGH,0,L,t,tag,"");
         }
      }
      if (which<1)
      {
         i=0; 
         while(AMRD_hyperbolic_vars[i]) PAMR_save_gfn(AMRD_hyperbolic_vars[i++],PAMR_MGH,0,L,t,tag,"");
         PAMR_save_gfn("cmask",PAMR_MGH,0,L,t,tag,"");
         i=0; 
         while(AMRD_MGH_work_vars[i]) PAMR_save_gfn(AMRD_MGH_work_vars[i++],PAMR_MGH,0,L,t,tag,"");
         i=0; 
         while(AMRD_AMRH_work_in_MGH_vars[i]) PAMR_save_gfn(AMRD_AMRH_work_in_MGH_vars[i++],PAMR_MGH,0,L,t,tag,"");
      }
   }
}
