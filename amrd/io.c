...
void init_context(char *pfile)
{
   ...
   int interp_buffer=2,gdm_method,stage,sgfn,offset;
   ...
   char *amr_inject_v[AMRD_MAX_VARS];
   char *amr_interp2_v[AMRD_MAX_VARS];
   char *amr_interp4_v[AMRD_MAX_VARS];
   char *amr_sync_v[AMRD_MAX_VARS];
   char *amr_transfer2_v[AMRD_MAX_VARS];
   char *amr_transfer4_v[AMRD_MAX_VARS];

   char *mg_interp2_v[AMRD_MAX_VARS];
   char *mg_hw_restr_v[AMRD_MAX_VARS];
   char *mg_fw_restr_v[AMRD_MAX_VARS];
   char *mg_sync_v[AMRD_MAX_VARS];

   char *AMRD_TRE_vars[AMRD_MAX_VARS];
   ...
   char *even_vars_x0min[AMRD_MAX_VARS],*even_vars_x1min[AMRD_MAX_VARS],*even_vars_x2min[AMRD_MAX_VARS];
   char *even_vars_x0max[AMRD_MAX_VARS],*even_vars_x1max[AMRD_MAX_VARS],*even_vars_x2max[AMRD_MAX_VARS];

   int num_odd_vars_x0min,num_odd_vars_x1min,num_odd_vars_x2min;
   int num_odd_vars_x0max,num_odd_vars_x1max,num_odd_vars_x2max;
   char *odd_vars_x0min[AMRD_MAX_VARS],*odd_vars_x1min[AMRD_MAX_VARS],*odd_vars_x2min[AMRD_MAX_VARS];
   char *odd_vars_x0max[AMRD_MAX_VARS],*odd_vars_x1max[AMRD_MAX_VARS],*odd_vars_x2max[AMRD_MAX_VARS];
   ...
   AMRD_periodic[1]=0;
   AMRD_periodic[2]=0;
   for (i=0; i<AMRD_MAX_VARS; i++) 
   {
      AMRD_save_mg_vars[i]=0;
      AMRD_TRE_vars[i]=0;
      AMRD_f_tre_vars[i]=0;
      amr_inject_v[i]=0;
      amr_interp2_v[i]=0;
      ...
      mg_sync_v[i]=0;
      mg_interp2_v[i]=0;
      AMRD_tnp1_diss_vars[i]=0;
      AMRD_ex_repop1_vars[i]=0;
      AMRD_ex_repop2_vars[i]=0;
      AMRD_ex_repop3_vars[i]=0;
      AMRD_ex_repop4_vars[i]=0;
      AMRD_tn_diss_vars[i]=0;
      AMRD_tnp1_liipb_vars[i]=0;
      AMRD_tnp1_liiab_vars[i]=0;
      AMRD_tnp1_liibb_vars[i]=0;
      AMRD_rg_diss_vars[i]=0;
      AMRD_interp_AMR_bdy_vars[i]=0;
      AMRD_regrid_script_name=0;
      AMRD_regrid_script=0;
      for (j=0; j<AMRD_MAX_TIMES; j++) AMRD_save_n_vars[j][i]=0;
      even_vars_x0min[i]=0;
      even_vars_x1min[i]=0;
      even_vars_x2min[i]=0;
      even_vars_x0max[i]=0;
   ...
   AMRD_num_elliptic_vars=0;
   AMRD_num_elliptic_vars_t0=0;
   AMRD_num_hyperbolic_vars=0;
   AMRD_num_AMRH_work_vars=0;
   AMRD_num_MGH_work_vars=0;
   AMRD_num_f_tre_vars=0;
   AMRD_num_tnp1_diss_vars=0;
   AMRD_num_ex_repop1_vars=0;
   AMRD_num_ex_repop2_vars=0;
   AMRD_num_ex_repop3_vars=0;
   AMRD_num_ex_repop4_vars=0;
   AMRD_num_tn_diss_vars=0;
   AMRD_num_tnp1_liipb_vars=0;
   AMRD_num_tnp1_liiab_vars=0;
   AMRD_num_tnp1_liibb_vars=0;
   AMRD_num_rg_diss_vars=0;
   AMRD_tnum_rg_diss_vars=0;
   AMRD_num_interp_AMR_bdy_vars=0;
   AMRD_num_MG_cnst_data_vars=0;
   AMRD_eps_diss=0;
   AMRD_diss_bdy=0;
   ...
   num_even_vars_x0min=0; AMRD_int_param(pfile,"num_even_vars_x0min",&num_even_vars_x0min,1);

   AMRD_num_hyperbolic_vars=0; AMRD_int_param(pfile,"num_hyperbolic_vars",&AMRD_num_hyperbolic_vars,1);
   if (AMRD_num_hyperbolic_vars>=AMRD_MAX_VARS) 
      AMRD_stop("num_hyperbolic_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (AMRD_num_hyperbolic_vars) AMRD_str_param(pfile,"hyperbolic_vars",AMRD_hyperbolic_vars,AMRD_num_hyperbolic_vars); 
   AMRD_hyperbolic_vars[AMRD_num_hyperbolic_vars]=0;

   AMRD_num_elliptic_vars=0; AMRD_int_param(pfile,"num_elliptic_vars",&AMRD_num_elliptic_vars,1);
   if (AMRD_num_elliptic_vars>=AMRD_MAX_VARS) 
      AMRD_stop("num_elliptic_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (AMRD_num_elliptic_vars) AMRD_str_param(pfile,"elliptic_vars",AMRD_elliptic_vars,AMRD_num_elliptic_vars); 
   AMRD_elliptic_vars[AMRD_num_elliptic_vars]=0;

   AMRD_num_elliptic_vars_t0=0; AMRD_int_param(pfile,"num_elliptic_vars_t0",&AMRD_num_elliptic_vars_t0,1);
   if (AMRD_num_elliptic_vars_t0+AMRD_num_elliptic_vars>=AMRD_MAX_VARS) 
      AMRD_stop("num_elliptic_vars_t0 is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (AMRD_num_elliptic_vars_t0) AMRD_str_param(pfile,"elliptic_vars_t0",AMRD_elliptic_vars_t0,AMRD_num_elliptic_vars_t0); 
   AMRD_elliptic_vars_t0[AMRD_num_elliptic_vars_t0]=0;

   AMRD_num_AMRH_work_vars=0; AMRD_int_param(pfile,"num_AMRH_work_vars",&AMRD_num_AMRH_work_vars,1);
   if (AMRD_num_AMRH_work_vars>=AMRD_MAX_VARS) 
      AMRD_stop("num_AMRH_work_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (AMRD_num_AMRH_work_vars) AMRD_str_param(pfile,"AMRH_work_vars",AMRD_AMRH_work_vars,AMRD_num_AMRH_work_vars); 
   AMRD_AMRH_work_vars[AMRD_num_AMRH_work_vars]=0;

   AMRD_num_AMRH_work_in_MGH_vars=0; AMRD_int_param(pfile,"num_AMRH_work_in_MGH_vars",&AMRD_num_AMRH_work_in_MGH_vars,1);
   if (AMRD_num_AMRH_work_in_MGH_vars>=AMRD_MAX_VARS) 
      AMRD_stop("num_AMRH_work_in_MGH_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (AMRD_num_AMRH_work_in_MGH_vars) AMRD_str_param(pfile,"AMRH_work_in_MGH_vars",AMRD_AMRH_work_in_MGH_vars,AMRD_num_AMRH_work_in_MGH_vars); 
   AMRD_AMRH_work_in_MGH_vars[AMRD_num_AMRH_work_in_MGH_vars]=0;

   AMRD_num_MGH_work_vars=0; AMRD_int_param(pfile,"num_MGH_work_vars",&AMRD_num_MGH_work_vars,1);
   if (AMRD_num_MGH_work_vars>=AMRD_MAX_VARS) 
      AMRD_stop("num_MGH_work_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (AMRD_num_MGH_work_vars) AMRD_str_param(pfile,"MGH_work_vars",AMRD_MGH_work_vars,AMRD_num_MGH_work_vars); 
   AMRD_MGH_work_vars[AMRD_num_MGH_work_vars]=0;

   AMRD_num_MG_cnst_data_vars=0; AMRD_int_param(pfile,"num_MG_cnst_data_vars",&AMRD_num_MG_cnst_data_vars,1);
   ...
   if (AMRD_num_MG_cnst_data_vars) AMRD_str_param(pfile,"MG_cnst_data_vars",AMRD_MG_cnst_data_vars,AMRD_num_MG_cnst_data_vars); 
   AMRD_MG_cnst_data_vars[AMRD_num_MG_cnst_data_vars]=0;

   AMRD_num_ex_repop1_vars=0; AMRD_int_param(pfile,"num_ex_repop1_vars",&AMRD_num_ex_repop1_vars,1);
   ...
   if (AMRD_num_ex_repop1_vars) AMRD_str_param(pfile,"ex_repop1_vars",AMRD_ex_repop1_vars,AMRD_num_ex_repop1_vars); 
   AMRD_ex_repop1_vars[AMRD_num_ex_repop1_vars]=0;

   AMRD_num_ex_repop2_vars=0; AMRD_int_param(pfile,"num_ex_repop2_vars",&AMRD_num_ex_repop2_vars,1);
   ...
   if (AMRD_num_ex_repop2_vars) AMRD_str_param(pfile,"ex_repop2_vars",AMRD_ex_repop2_vars,AMRD_num_ex_repop2_vars); 
   AMRD_ex_repop2_vars[AMRD_num_ex_repop2_vars]=0;

   AMRD_num_ex_repop3_vars=0; AMRD_int_param(pfile,"num_ex_repop3_vars",&AMRD_num_ex_repop3_vars,1);
   ...
   if (AMRD_num_ex_repop3_vars) AMRD_str_param(pfile,"ex_repop3_vars",AMRD_ex_repop3_vars,AMRD_num_ex_repop3_vars); 
   AMRD_ex_repop3_vars[AMRD_num_ex_repop3_vars]=0;

   AMRD_num_ex_repop4_vars=0; AMRD_int_param(pfile,"num_ex_repop4_vars",&AMRD_num_ex_repop4_vars,1);
   ...
   if (AMRD_num_ex_repop4_vars) AMRD_str_param(pfile,"ex_repop4_vars",AMRD_ex_repop4_vars,AMRD_num_ex_repop4_vars); 
   AMRD_ex_repop4_vars[AMRD_num_ex_repop4_vars]=0;

   AMRD_num_tnp1_diss_vars=0; AMRD_int_param(pfile,"num_tnp1_diss_vars",&AMRD_num_tnp1_diss_vars,1);
   ...
   if (AMRD_num_tnp1_diss_vars) AMRD_str_param(pfile,"tnp1_diss_vars",AMRD_tnp1_diss_vars,AMRD_num_tnp1_diss_vars); 
   AMRD_tnp1_diss_vars[AMRD_num_tnp1_diss_vars]=0;
   ...
   AMRD_num_tn_diss_vars=0; AMRD_int_param(pfile,"num_tn_diss_vars",&AMRD_num_tn_diss_vars,1);
   ...
   if (AMRD_num_tn_diss_vars) AMRD_str_param(pfile,"tn_diss_vars",AMRD_tn_diss_vars,AMRD_num_tn_diss_vars); 
   AMRD_tn_diss_vars[AMRD_num_tn_diss_vars]=0;
   ...
   AMRD_num_tnp1_liipb_vars=0; AMRD_int_param(pfile,"num_tnp1_liipb_vars",&AMRD_num_tnp1_liipb_vars,1);
   ...
   if (AMRD_num_tnp1_liipb_vars) AMRD_str_param(pfile,"tnp1_liipb_vars",AMRD_tnp1_liipb_vars,AMRD_num_tnp1_liipb_vars); 
   AMRD_tnp1_liipb_vars[AMRD_num_tnp1_liipb_vars]=0;

   AMRD_num_tnp1_liiab_vars=0; AMRD_int_param(pfile,"num_tnp1_liiab_vars",&AMRD_num_tnp1_liiab_vars,1);
   ...
   if (AMRD_num_tnp1_liiab_vars) AMRD_str_param(pfile,"tnp1_liiab_vars",AMRD_tnp1_liiab_vars,AMRD_num_tnp1_liiab_vars); 
   AMRD_tnp1_liiab_vars[AMRD_num_tnp1_liiab_vars]=0;

   AMRD_num_tnp1_liibb_vars=0; AMRD_int_param(pfile,"num_tnp1_liibb_vars",&AMRD_num_tnp1_liibb_vars,1);
   ...
   if (AMRD_num_tnp1_liibb_vars) AMRD_str_param(pfile,"tnp1_liibb_vars",AMRD_tnp1_liibb_vars,AMRD_num_tnp1_liibb_vars); 
   AMRD_tnp1_liibb_vars[AMRD_num_tnp1_liibb_vars]=0;

   AMRD_num_rg_diss_vars=0; AMRD_int_param(pfile,"num_rg_diss_vars",&AMRD_num_rg_diss_vars,1);
   ...
   if (AMRD_num_rg_diss_vars) AMRD_str_param(pfile,"rg_diss_vars",AMRD_rg_diss_vars,AMRD_num_rg_diss_vars); 
   AMRD_rg_diss_vars[AMRD_num_rg_diss_vars]=0;
   AMRD_real_param(pfile,"rg_eps_diss",&AMRD_rg_eps_diss,1);
   
   AMRD_num_interp_AMR_bdy_vars=0; AMRD_int_param(pfile,"num_interp_AMR_bdy_vars",&AMRD_num_interp_AMR_bdy_vars,1);
   ...
   if (AMRD_num_interp_AMR_bdy_vars) AMRD_str_param(pfile,"interp_AMR_bdy_vars",AMRD_interp_AMR_bdy_vars,AMRD_num_interp_AMR_bdy_vars); 
   AMRD_interp_AMR_bdy_vars[AMRD_num_interp_AMR_bdy_vars]=0;
   ...
   i=0; AMRD_int_param(pfile,"num_TRE_vars",&i,1);
   if (i>AMRD_MAX_VARS) AMRD_stop("num_TRE_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"TRE_vars",AMRD_TRE_vars,i); 
   ...
   i=0; AMRD_int_param(pfile,"num_save_mg_vars",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_save_mg_vars is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"save_mg_vars",AMRD_save_mg_vars,i); 

   for (j=0; j<AMRD_MAX_TIMES; j++)
   {
      sprintf(buffer,"num_save_%i_vars",j+1);
      i=0; AMRD_int_param(pfile,buffer,&i,1);
      if (i>=AMRD_MAX_VARS) AMRD_stop(buffer," is greater than maximum allowed ... change AMRD_MAX_VARS and recompile"); 
      sprintf(buffer,"save_%i_vars",j+1);
      if (i) AMRD_str_param(pfile,buffer,&(AMRD_save_n_vars[j][0]),i); 
   }
   
   i=0; AMRD_int_param(pfile,"num_amr_inject",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_amr_inject is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"amr_inject",amr_inject_v,i); 

   i=0; AMRD_int_param(pfile,"num_amr_interp2",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_amr_interp2 is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"amr_interp2",amr_interp2_v,i); 

   i=0; AMRD_int_param(pfile,"num_amr_interp4",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_amr_interp4 is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"amr_interp4",amr_interp4_v,i); 

   i=0; AMRD_int_param(pfile,"num_amr_sync",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_amr_sync is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"amr_sync",amr_sync_v,i); 

   i=0; AMRD_int_param(pfile,"num_amr_transfer2",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_amr_transfer2 is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"amr_transfer2",amr_transfer2_v,i); 

   i=0; AMRD_int_param(pfile,"num_amr_transfer4",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_amr_transfer4 is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"amr_transfer4",amr_transfer4_v,i); 

   i=0; AMRD_int_param(pfile,"num_mg_interp2",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_mg_interp2 is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"mg_interp2",mg_interp2_v,i); 

   i=0; AMRD_int_param(pfile,"num_mg_hw_restr",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_mg_hw_restr is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"mg_hw_restr",mg_hw_restr_v,i); 

   i=0; AMRD_int_param(pfile,"num_mg_fw_restr",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_mg_fw_restr is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"mg_fw_restr",mg_fw_restr_v,i); 

   i=0; AMRD_int_param(pfile,"num_mg_sync",&i,1);
   if (i>=AMRD_MAX_VARS) AMRD_stop("num_mg_sync is greater than maximum allowed ... change AMRD_MAX_VARS and recompile",0); 
   if (i) AMRD_str_param(pfile,"mg_sync",mg_sync_v,i); 
   ...
   if (!(pamr_context=PAMR_init_context(0,0,AMRD_dim,AMRD_base_shape,AMRD_base_bbox,AMRD_cp_restore_fname,-1)))
   ...
   // have been restored ... except AMRD_f_tre_vars ... idiot
   // =========================================================================
   if (AMRD_cp_restart)
   {
      i=0;j=0;
      while(AMRD_hyperbolic_vars[i])
      {
         if (in_list(AMRD_hyperbolic_vars[i],AMRD_TRE_vars))
	 {
   ...
   if (num_even_vars_x0min) AMRD_str_param(pfile,"even_vars_x0min",even_vars_x0min,num_even_vars_x0min); 
   even_vars_x0min[num_even_vars_x0min]=0;

   num_even_vars_x1min=0; AMRD_int_param(pfile,"num_even_vars_x1min",&num_even_vars_x1min,1);
   ...
   if (num_even_vars_x1min) AMRD_str_param(pfile,"even_vars_x1min",even_vars_x1min,num_even_vars_x1min); 
   even_vars_x1min[num_even_vars_x1min]=0;

   num_even_vars_x2min=0; AMRD_int_param(pfile,"num_even_vars_x2min",&num_even_vars_x2min,1);
   ...
   if (num_even_vars_x2min) AMRD_str_param(pfile,"even_vars_x2min",even_vars_x2min,num_even_vars_x2min); 
   even_vars_x2min[num_even_vars_x2min]=0;

   num_even_vars_x0max=0; AMRD_int_param(pfile,"num_even_vars_x0max",&num_even_vars_x0max,1);
   ...
   if (num_even_vars_x0max) AMRD_str_param(pfile,"even_vars_x0max",even_vars_x0max,num_even_vars_x0max); 
   even_vars_x0max[num_even_vars_x0max]=0;

   num_even_vars_x1max=0; AMRD_int_param(pfile,"num_even_vars_x1max",&num_even_vars_x1max,1);
   ...
   if (num_even_vars_x1max) AMRD_str_param(pfile,"even_vars_x1max",even_vars_x1max,num_even_vars_x1max); 
   even_vars_x1max[num_even_vars_x1max]=0;

   num_even_vars_x2max=0; AMRD_int_param(pfile,"num_even_vars_x2max",&num_even_vars_x2max,1);
   ...
   if (num_even_vars_x2max) AMRD_str_param(pfile,"even_vars_x2max",even_vars_x2max,num_even_vars_x2max); 
   even_vars_x2max[num_even_vars_x2max]=0;

   num_odd_vars_x0min=0; AMRD_int_param(pfile,"num_odd_vars_x0min",&num_odd_vars_x0min,1);
   ...
   if (num_odd_vars_x0min) AMRD_str_param(pfile,"odd_vars_x0min",odd_vars_x0min,num_odd_vars_x0min); 
   odd_vars_x0min[num_odd_vars_x0min]=0;

   num_odd_vars_x1min=0; AMRD_int_param(pfile,"num_odd_vars_x1min",&num_odd_vars_x1min,1);
   ...
   if (num_odd_vars_x1min) AMRD_str_param(pfile,"odd_vars_x1min",odd_vars_x1min,num_odd_vars_x1min); 
   odd_vars_x1min[num_odd_vars_x1min]=0;

   num_odd_vars_x2min=0; AMRD_int_param(pfile,"num_odd_vars_x2min",&num_odd_vars_x2min,1);
   ...
   if (num_odd_vars_x2min) AMRD_str_param(pfile,"odd_vars_x2min",odd_vars_x2min,num_odd_vars_x2min); 
   odd_vars_x2min[num_odd_vars_x2min]=0;

   num_odd_vars_x0max=0; AMRD_int_param(pfile,"num_odd_vars_x0max",&num_odd_vars_x0max,1);
   ...
   if (num_odd_vars_x0max) AMRD_str_param(pfile,"odd_vars_x0max",odd_vars_x0max,num_odd_vars_x0max); 
   odd_vars_x0max[num_odd_vars_x0max]=0;

   num_odd_vars_x1max=0; AMRD_int_param(pfile,"num_odd_vars_x1max",&num_odd_vars_x1max,1);
   ...
   if (num_odd_vars_x1max) AMRD_str_param(pfile,"odd_vars_x1max",odd_vars_x1max,num_odd_vars_x1max); 
   odd_vars_x1max[num_odd_vars_x1max]=0;

   num_odd_vars_x2max=0; AMRD_int_param(pfile,"num_odd_vars_x2max",&num_odd_vars_x2max,1);
   ...
   if (num_odd_vars_x2max) AMRD_str_param(pfile,"odd_vars_x2max",odd_vars_x2max,num_odd_vars_x2max); 
   odd_vars_x2max[num_odd_vars_x2max]=0;
   ...
   if (AMRD_echo_params && my_rank==0) printf("\n==============================\n");
   var=AMRD_hyperbolic_vars[i];
   stage=0;
   if (!var)
   {
      stage++;
      var=AMRD_elliptic_vars[i];
      if (!var)
      {
         stage++;
         var=AMRD_AMRH_work_vars[i];
         if (!var)
         {
            stage++;
            var=AMRD_MGH_work_vars[i];
            if  (!var)
            {
               stage++;
               var=AMRD_elliptic_vars_t0[i];
   ...
   while(var)
   {
      ...
      switch(stage)
      {
         ...
         case 2: // AMR work
            ...
            mg_noinj_to_amr=1;
            if (in_list(var,AMRD_AMRH_work_in_MGH_vars)) in_mgh=1;
            break;
         case 3: // MG work
      ...
      if (in_list(var,amr_interp2_v)) amr_interp=amr_bdy_interp=PAMR_SECOND_ORDER;
      if (in_list(var,amr_interp4_v)) amr_interp=amr_bdy_interp=PAMR_FOURTH_ORDER;
      if (in_list(var,amr_inject_v)) amr_inject=PAMR_STRAIGHT_INJECT;
      if (in_list(var,amr_sync_v)) amr_sync=PAMR_SYNC;
      if (in_list(var,mg_sync_v)) mg_sync=PAMR_SYNC;
      if (in_list(var,mg_hw_restr_v)) mg_inject=PAMR_HW_RESTR;
      if (in_list(var,mg_fw_restr_v)) mg_inject=PAMR_FW_RESTR;
      if (in_list(var,amr_transfer2_v)) regrid_transfer=PAMR_SECOND_ORDER;
      if (in_list(var,amr_transfer4_v)) regrid_transfer=PAMR_FOURTH_ORDER;

      phys_bdy_type[0]=phys_bdy_type[1]=phys_bdy_type[2]=phys_bdy_type[3]=phys_bdy_type[4]=phys_bdy_type[5]=PAMR_UNKNOWN;
      if (in_list(var,even_vars_x0min)) phys_bdy_type[0]=PAMR_EVEN;
      if (in_list(var,even_vars_x1min)) phys_bdy_type[2]=PAMR_EVEN;
      if (in_list(var,even_vars_x2min)) phys_bdy_type[4]=PAMR_EVEN;
      if (in_list(var,even_vars_x0max)) phys_bdy_type[1]=PAMR_EVEN;
      if (in_list(var,even_vars_x1max)) phys_bdy_type[3]=PAMR_EVEN;
      if (in_list(var,even_vars_x2max)) phys_bdy_type[5]=PAMR_EVEN;
      if (in_list(var,odd_vars_x0min)) phys_bdy_type[0]=PAMR_ODD;
      if (in_list(var,odd_vars_x1min)) phys_bdy_type[2]=PAMR_ODD;
      if (in_list(var,odd_vars_x2min)) phys_bdy_type[4]=PAMR_ODD;
      if (in_list(var,odd_vars_x0max)) phys_bdy_type[1]=PAMR_ODD;
      if (in_list(var,odd_vars_x1max)) phys_bdy_type[3]=PAMR_ODD;
      if (in_list(var,odd_vars_x2max)) phys_bdy_type[5]=PAMR_ODD;

      sgfn=PAMR_def_var_full(var,in_amrh,in_mgh,num_tl,amr_inject,amr_interp,
                             amr_bdy_interp,amr_sync,mg_inject,mg_interp,
                             mg_sync,mg_noinj_to_amr,regrid_transfer,phys_bdy_type);
      ...
      if (stage==0)
      {
         ...
         if (stage==0 && in_list(var,AMRD_TRE_vars))
         {
            amr_inject=PAMR_NO_INJECT;
            amr_interp=PAMR_NO_INTERP;
            amr_bdy_interp=PAMR_NO_INTERP;
      ...
      if (stage==1 || stage==4)
      {
         ...
         mg_interp=PAMR_NO_INTERP;
         if (in_list(buffer,mg_fw_restr_v)) mg_inject=PAMR_FW_RESTR;
         if (in_list(var,mg_interp2_v)) mg_interp=PAMR_SECOND_ORDER;
         mg_sync=PAMR_NO_SYNC;
         regrid_transfer=PAMR_NO_TRANSFER;
         in_amrh=0; in_mgh=1; num_tl=0;
         mg_noinj_to_amr=0;
         ...
         //====================================================================
         // NOTE: we use f_lop to interpolate f's CGC's in the vcycle ...
         //       hence we set mg_interp for f_lop
         //====================================================================
         strcpy(buffer,var);
         strcat(buffer,"_lop");
         mg_inject=PAMR_NO_INJECT;
         ...
         if (stage==1)
         {
            ...
            if (in_list(buffer,amr_interp2_v)) amr_interp=amr_bdy_interp=PAMR_SECOND_ORDER;
            regrid_transfer=PAMR_FOURTH_ORDER;
            if (in_list(buffer,amr_transfer2_v)) regrid_transfer=PAMR_SECOND_ORDER;
            in_amrh=1; in_mgh=0; num_tl=1;
            mg_noinj_to_amr=1;
      ...
      switch(stage)
      {
         case 0: var=AMRD_hyperbolic_vars[i]; break;
         case 1: var=AMRD_elliptic_vars[i]; break;
         case 2: var=AMRD_AMRH_work_vars[i]; break;
         case 3: var=AMRD_MGH_work_vars[i]; break;
         case 4: var=AMRD_elliptic_vars_t0[i]; break;
      }
   ...
   {
      strcpy(buffer,"chr");
      AMRD_chr_gfn=PAMR_def_var_full(buffer,in_amrh,in_mgh,num_tl,amr_inject,amr_interp,
                               amr_bdy_interp,amr_sync,mg_inject,mg_interp,
                               mg_sync,mg_noinj_to_amr,regrid_transfer,phys_bdy_type);
      AMRD_chr_mg_gfn=AMRD_chr_gfn+1;
      if (AMRD_echo_params && my_rank==0) printf("%s:\n\n",buffer);
   }
   else { AMRD_chr_gfn=0; AMRD_chr_mg_gfn=0; }
   ...
   for (i=0; i<AMRD_MAX_VARS; i++) 
   {
      if (AMRD_TRE_vars[i]) free(AMRD_TRE_vars[i]);
      if (amr_inject_v[i]) free(amr_inject_v[i]);
      if (amr_interp2_v[i]) free(amr_interp2_v[i]);
      if (amr_interp4_v[i]) free(amr_interp4_v[i]);
      if (amr_sync_v[i]) free(amr_sync_v[i]);
      if (amr_transfer2_v[i]) free(amr_transfer2_v[i]);
      if (amr_transfer4_v[i]) free(amr_transfer4_v[i]);
      if (mg_hw_restr_v[i]) free(mg_hw_restr_v[i]);
      if (mg_fw_restr_v[i]) free(mg_fw_restr_v[i]);
      if (mg_sync_v[i]) free(mg_sync_v[i]);
      if (mg_interp2_v[i]) free(mg_interp2_v[i]);
      if (even_vars_x0min[i]) free(even_vars_x0min[i]);
      if (even_vars_x1min[i]) free(even_vars_x1min[i]);
      if (even_vars_x2min[i]) free(even_vars_x2min[i]);
      if (even_vars_x0max[i]) free(even_vars_x0max[i]);
      if (even_vars_x1max[i]) free(even_vars_x1max[i]);
      if (even_vars_x2max[i]) free(even_vars_x2max[i]);
      if (odd_vars_x0min[i]) free(odd_vars_x0min[i]);
      if (odd_vars_x1min[i]) free(odd_vars_x1min[i]);
      if (odd_vars_x2min[i]) free(odd_vars_x2min[i]);
      if (odd_vars_x0max[i]) free(odd_vars_x0max[i]);
      if (odd_vars_x1max[i]) free(odd_vars_x1max[i]);
      if (odd_vars_x2max[i]) free(odd_vars_x2max[i]);
   }
   ...
   for (i=0; i<AMRD_num_tnp1_diss_vars; i++)
   {
      ...
      PAMR_get_var_attribs(AMRD_tnp1_diss_vars[i],inum,inum,inum,inum,
                           inum,inum,inum,inum,inum,inum,inum,inum,&AMRD_tnp1_diss_pbt[i][0]);
   }
   if (AMRD_num_evo_tl>2 && AMRD_diss_all_past) mod=AMRD_num_evo_tl-1; else mod=1;

   for (i=0; i<(AMRD_num_tn_diss_vars*mod); i++) 
   {
      if (AMRD_num_evo_tl<2 || (  !(in_list(AMRD_tn_diss_vars[i%AMRD_num_tn_diss_vars],AMRD_hyperbolic_vars))
                                &&!(in_list(AMRD_tn_diss_vars[i%AMRD_num_tn_diss_vars],AMRD_elliptic_vars)) ) )
      {
      ...
      PAMR_get_var_attribs(AMRD_tn_diss_vars[i%AMRD_num_tn_diss_vars],inum,inum,inum,inum,
                           inum,inum,inum,inum,inum,inum,inum,inum,&AMRD_tn_diss_pbt[i][0]);
   }
   ...
   for (i=0,ii=0; i<AMRD_num_rg_diss_vars; i++) 
   {
      ...
      PAMR_get_var_attribs(AMRD_rg_diss_vars[i],inum,inum,&num_tl,inum,
                           inum,inum,inum,inum,inum,inum,inum,inum,&AMRD_rg_diss_pbt[ii][0]);
      ii++;
   ...
   for (i=0; i<AMRD_num_MG_cnst_data_vars; i++) AMRD_MG_cnst_data_gfn[i]=PAMR_get_gfn(AMRD_MG_cnst_data_vars[i],PAMR_MGH,0);
   ...
   strcpy(buffer,"chr");
   if (AMRD_do_ex) PAMR_excision_on(buffer,app_fill_ex_mask,AMRD_ex,1);
...
void amrd_copy_block(char *p, char **q, int n, int dir, int *tot_size)
{
   ...
   if ((*tot_size+n) > (AMRD_CP_MAX_SIZE+AMRD_cp_data_size)) 
...
int amrd_do_cp(char *cp_file, int dir)
{
   ...
   if (dir==AMRD_CP_RESTORE)
   {
      ...
      size0*=sizeof(double);
      if (!(data0=data=malloc(size0))) { printf("PAMR_do_cp: error ... out of memory\n"); goto fin;}
